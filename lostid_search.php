<?php
	...
	$result=zb_query("select * from ".$member_table." where email='$email' and jumin=CONCAT('*', UPPER(SHA1(UNHEX(SHA1('$jumin1"."$jumin2')))))",$connect) or Error(zb_error());
	...
	if(!mysql_num_rows($result)) Error("입력하신 정보에 해당하는 회원이 없습니다.<br><br>다시 한번확인하여 주시기 바랍니다");
		...
		zb_query("update $member_table set password=CONCAT('*', UPPER(SHA1(UNHEX(SHA1('$temp'))))) where no='$data[no]'",$connect) or Error(zb_error());
